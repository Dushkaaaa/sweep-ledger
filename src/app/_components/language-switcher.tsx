...
const publicLangSegments = ["uk", "en", "pl", "de"] as const;

function getPublicLangFromPathname(pathname: string) {
  const firstSegment = pathname.split("/")[1];
  return publicLangSegments.includes(firstSegment as (typeof publicLangSegments)[number])
    ? firstSegment
    : null;
}
...
export function LanguageSwitcher({ variant = "light" }: LanguageSwitcherProps) {
  ...
  const currentPublicLang = getPublicLangFromPathname(pathname);

  // async function handleLanguageChange(nextLanguage: typeof language) {
  //   // Публічна /[lang] сторінка — просто переходимо на інший URL
  //   if (currentPublicLang) {
  //     const restOfPath = pathname.slice(`/${currentPublicLang}`.length);
  //     router.push(`/${nextLanguage}${restOfPath}`);
  //     return;
  //   }

  //   // Workspace / залогінений юзер — як і раніше
  //   setLanguage(nextLanguage);
